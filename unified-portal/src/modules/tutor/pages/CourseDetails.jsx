...
export default function CourseDetails() {
  ...
  const fetchData = async () => {
    try {
      ...
      const mods =
        c.modules?.length && typeof c.modules[0] === "object"
          ? c.modules
          : mRes.data.data || [];
      setModules(mods);
    } catch {
  ...
  const editModule = async (modId, e) => {
    e.stopPropagation();
    try {
      await apiClient.put(`/courses/${courseId}/modules/${modId}`, {
        title: editTitle,
      });
      setEditingId(null);
      fetchData();
  ...
  const deleteModule = async (modId, e) => {
    ...
    try {
      await apiClient.delete(`/courses/${courseId}/modules/${modId}`);
      fetchData();
    } catch {
  ...
  const handleCoverUpload = async (e) => {
    ...
    try {
      ...
      // Store only the relative path, not the full resolved URL
      setCoverUrl(url);
    ...
    <div style={S.page}>
      ...
      <div style={S.header}>
        <div style={S.headerLeft}>
          ...
          <div style={S.coverThumb}>
            {course.coverImage || course.thumbnail ? (
              <img
                src={resolveCover(course.coverImage || course.thumbnail)}
                alt="cover"
                style={S.coverThumbImg}
        ...
        <div style={S.headerActions}>
          ...
          <button onClick={() => setShowModuleForm(true)} style={S.addBtn}>
            + Add Module
          </button>
        </div>
      </div>
      ...
      {showCoverEdit && (
        <div style={S.coverPanel}>
          ...
          <div style={S.panelActions}>
            ...
            <button onClick={() => setShowCoverEdit(false)} style={S.cancelBtn}>
              Cancel
            </button>
          </div>
        </div>
        ...
        {modules.length === 0 ? (
          <div style={S.empty}>
            <p style={{ color: "var(--text-secondary)", fontSize: 15 }}>
              No modules yet. Start structuring your course!
            </p>
          </div>
        ) : (
          modules.map((mod, i) => (
            <div key={mod._id}>
              {editingId === mod._id ? (
                <div style={S.editRow}>
                  ...
                  />
                  <button
                    onClick={(e) => editModule(mod._id, e)}
                    style={S.saveBtn}
                  >
                    Save
                  </button>
                ...
                <div
                  style={S.moduleCard}
                  onClick={() =>
                    navigate(`/tutor/course/${courseId}/module/${mod._id}`)
                  }
                >
                  ...
                  <div style={S.moduleActions}>
                    ...
                    <button
                      onClick={(e) => deleteModule(mod._id, e)}
                      style={{
                        ...S.iconBtn,
                        color: "#f87171",
