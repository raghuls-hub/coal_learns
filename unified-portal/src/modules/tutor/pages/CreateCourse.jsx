...
function StyledSelect({ value, onChange, options, label, required }) {
  return (
    <div style={F.group}>
      {label && <label style={F.label}>{label}{required && <span style={F.req}> *</span>}</label>}
      <div style={F.selectWrap}>
        <select value={value} onChange={onChange} required={required} style={F.select}>
          {options.map(o => <option key={o.value} value={o.value}>{o.label}</option>)}
        </select>
        <svg style={F.selectArrow} width="12" height="12" viewBox="0 0 24 24" fill="none">
        ...
        <div style={S.formCard}>
          ...
          <form onSubmit={handleSubmit} style={S.form}>
            ...
            <div style={S.section}>
              ...
              <div style={F.group}>
                <label style={F.label}>Course Title <span style={F.req}>*</span></label>
                <input value={form.title} onChange={e => set('title', e.target.value)} required style={F.input} placeholder="e.g., Introduction to Web Development" />
              </div>
              <div style={F.group}>
                <label style={F.label}>Description <span style={F.req}>*</span></label>
                <textarea value={form.description} onChange={e => set('description', e.target.value)} required style={F.textarea} rows={5} placeholder="Describe what students will learn in this course…" />
              </div>
              <div style={S.row2}>
                <div style={F.group}>
                  <label style={F.label}>Category <span style={F.req}>*</span></label>
                  <div style={F.selectWrap}>
                    <select value={form.category} onChange={e => set('category', e.target.value)} required style={F.select}>
            ...
            <div style={S.section}>
              ...
              {coverMode === 'url' ? (
                ...
                <div style={S.uploadZone}>
                  <input type="file" accept="image/*" onChange={handleCoverUpload} id="cover-upload" style={{ display: 'none' }} />
                  <label htmlFor="cover-upload" style={S.uploadLabel}>
                    ...
                    <span style={{ fontSize: 12, color: 'var(--text-muted)' }}>PNG, JPG, WEBP up to 5MB</span>
                  </label>
                </div>
              )}
              {coverPreview && (
                <div style={S.coverPreviewWrap}>
                  <img src={coverPreview} alt="Cover preview" style={S.coverPreview} onError={() => setCoverPreview('')} />
                  <button type="button" onClick={() => { setCoverPreview(''); set('coverImage', ''); }} style={S.removeCoverBtn}>
...
const F = {
  ...
  req: { color: '#f87171' },
  input: { padding: '0.8rem 1rem', background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 10, color: 'var(--text-primary)', fontSize: 14, outline: 'none', width: '100%', transition: 'border-color 0.2s', fontFamily: 'inherit' },
  textarea: { padding: '0.8rem 1rem', background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 10, color: 'var(--text-primary)', fontSize: 14, outline: 'none', resize: 'vertical', minHeight: 120, fontFamily: 'inherit', width: '100%', lineHeight: 1.6 },
  selectWrap: { position: 'relative' },
  select: { width: '100%', padding: '0.8rem 2.5rem 0.8rem 1rem', background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 10, color: 'var(--text-primary)', fontSize: 14, outline: 'none', appearance: 'none', cursor: 'pointer', fontFamily: 'inherit' },
  selectArrow: { position: 'absolute', right: 12, top: '50%', transform: 'translateY(-50%)', pointerEvents: 'none' },
  inputPrefix: { position: 'relative' },
  prefix: { position: 'absolute', left: 12, top: '50%', transform: 'translateY(-50%)', fontSize: 14, color: 'var(--text-muted)', fontWeight: 600, pointerEvents: 'none' },
...
const S = {
  ...
  title: { fontSize: 26, fontWeight: 900, color: 'var(--text-primary)', letterSpacing: '-0.03em', marginBottom: '0.4rem' },
  ...
  section: { display: 'flex', flexDirection: 'column', gap: '1.25rem', paddingBottom: '2rem', borderBottom: '1px solid var(--border)' },
  sectionLabel: { fontSize: 11, fontWeight: 700, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.1em' },
  row2: { display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.25rem' },

  coverToggle: { display: 'flex', gap: 6, background: 'rgba(255,255,255,0.03)', border: '1px solid var(--border)', borderRadius: 10, padding: 4, width: 'fit-content' },
  toggleBtn: { padding: '0.45rem 1.1rem', background: 'transparent', border: 'none', borderRadius: 7, cursor: 'pointer', fontSize: 13, fontWeight: 600, color: 'var(--text-muted)' },
  toggleActive: { background: 'rgba(99,102,241,0.15)', color: '#818cf8' },

  uploadZone: { border: '2px dashed rgba(255,255,255,0.1)', borderRadius: 12, overflow: 'hidden' },
  uploadLabel: { display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8, padding: '2rem', cursor: 'pointer' },
  coverPreviewWrap: { display: 'flex', alignItems: 'flex-start', gap: '1rem' },
  coverPreview: { width: 200, height: 112, objectFit: 'cover', borderRadius: 10, border: '1px solid var(--border)' },
  removeCoverBtn: { display: 'flex', alignItems: 'center', gap: 5, padding: '0.4rem 0.875rem', background: 'rgba(239,68,68,0.08)', color: '#f87171', border: '1px solid rgba(239,68,68,0.2)', borderRadius: 8, cursor: 'pointer', fontSize: 12, fontWeight: 600 },
  ...
  infoTitle: { fontSize: 15, fontWeight: 800, color: '#818cf8', marginBottom: '1.25rem' },
  steps: { display: 'flex', flexDirection: 'column', gap: '0.875rem' },
  step: { display: 'flex', alignItems: 'center', gap: '0.875rem' },
  stepNum: { width: 28, height: 28, borderRadius: 8, background: 'rgba(99,102,241,0.12)', border: '1px solid rgba(99,102,241,0.25)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, fontWeight: 800, color: '#818cf8', flexShrink: 0, letterSpacing: '0.02em' },
};
