...
function buildMonthlyGrid(activityMap) {
  ...
  for (let m = 5; m >= 0; m--) {
    ...
    for (let d = 1; d <= daysInMonth; d++) {
      ...
      row[col] = { day: d, key, count: activityMap[key] || 0, isFuture: date > today };
      col++;
      if (col === 7) {
        rows.push(row);
    ...
    <div style={H.wrap}>
      <div style={H.header}>
        <div>
          ...
          <p style={H.sub}>{totalModules} chapter{totalModules !== 1 ? 's' : ''} completed · {totalDays} active day{totalDays !== 1 ? 's' : ''}</p>
        </div>
        <div style={H.legend}>
          <span style={H.legendLabel}>Less</span>
          {[0,1,2,3,4].map(i => (
            <div key={i} style={{ width: 12, height: 12, borderRadius: 3, background: getIntensityColor(i), border: '1px solid rgba(255,255,255,0.06)' }} />
          ))}
      ...
      <div style={H.grid}>
        {months.map(({ label, year, rows }) => (
          <div key={`${label}${year}`} style={H.monthBlock}>
            <p style={H.monthLabel}>{label} {year}</p>
            <div style={{ display: 'flex', gap: GAP, marginBottom: GAP }}>
              {DAY_ABBR.map((d, i) => (
                <div key={i} style={{ width: CELL, fontSize: 9, color: 'var(--text-muted)', textAlign: 'center', fontWeight: 600 }}>{d}</div>
              ))}
            </div>
            {rows.map((week, wi) => (
                  ...
                  return (
                    <div
                      ...
                      style={{
                        ...
                        background: cell.isFuture ? 'rgba(255,255,255,0.03)' : getIntensityColor(cell.count),
      ...
      {tooltip && (
        <div style={{ ...H.tooltip, left: tooltip.x + 12, top: tooltip.y - 44 }}>
          <strong>{tooltip.count} chapter{tooltip.count !== 1 ? 's' : ''}</strong>
          {' on '}{new Date(tooltip.key + 'T00:00:00').toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' })}
        </div>
      )}
    </div>
  );
...
const H = {
  wrap: { background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 16, padding: '1.75rem 2rem', marginBottom: '2.5rem', position: 'relative' },
  header: { display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '1.5rem', flexWrap: 'wrap', gap: '1rem' },
  title: { fontSize: 17, fontWeight: 800, color: 'var(--text-primary)', letterSpacing: '-0.02em', marginBottom: 4 },
  ...
  monthLabel: { fontSize: 11, fontWeight: 700, color: 'var(--text-secondary)', marginBottom: 6, textTransform: 'uppercase', letterSpacing: '0.06em' },
  tooltip: { position: 'fixed', background: '#1e293b', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 8, padding: '6px 10px', fontSize: 12, color: 'var(--text-primary)', pointerEvents: 'none', zIndex: 1000, whiteSpace: 'nowrap', boxShadow: '0 8px 24px rgba(0,0,0,0.4)' },
};
...
function CourseCover({ src, title, style }) {
  ...
  if (resolved && !err) {
    return <img src={resolved} alt={title} onError={() => setErr(true)} style={style} />;
  }
  const hue = (title || '').split('').reduce((a, c) => a + c.charCodeAt(0), 0) % 360;
  return (
    <div style={{ ...style, background: `linear-gradient(135deg, hsl(${hue},55%,16%) 0%, hsl(${(hue+40)%360},45%,10%) 100%)`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
...
export default function MyLearning() {
  ...
  if (loading) return (
    <div style={S.page}>
      <div style={S.grid}>{[...Array(3)].map((_, i) => <div key={i} className="skeleton" style={{ height: 260, borderRadius: 16 }} />)}</div>
    </div>
  );
  ...
  return (
    <div style={S.page}>
      {modal && (
        <div style={S.overlay} onClick={() => setModal(null)}>
          <div style={S.modal} onClick={e => e.stopPropagation()}>
            <div style={{ ...S.modalIconWrap, background: modal === 'deleted' ? 'rgba(239,68,68,0.1)' : 'rgba(245,158,11,0.1)', border: `1px solid ${modal === 'deleted' ? 'rgba(239,68,68,0.2)' : 'rgba(245,158,11,0.2)'}` }}>
              {modal === 'deleted'
                ? <svg width="22" height="22" viewBox="0 0 24 24" fill="none"><path d="M3 6h18M8 6V4h8v2M19 6l-1 14H6L5 6" stroke="#f87171" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/></svg>
                : <svg width="22" height="22" viewBox="0 0 24 24" fill="none"><circle cx="12" cy="12" r="10" stroke="#f59e0b" strokeWidth="1.5"/><path d="M12 8v4M12 16h.01" stroke="#f59e0b" strokeWidth="1.5" strokeLinecap="round"/></svg>
              }
            </div>
      ...
      <ActivityHeatmap activityMap={activityMap} totalDays={totalDays} totalModules={totalModules} />
        ...
        <div style={S.grid}>
          {enrollments.map(en => {
            ...
            const cert = certificates.find(c =>
              (c.course && en.course && c.course === en.course._id) || c.enrollment === en._id
            );
            const courseId = en.course?._id || en.course;

            return (
              <div key={en._id} style={S.card}>
                <CourseCover src={data.coverImage || data.thumbnail} title={data.title} style={S.cardCover} />
                ...
                <div style={S.cardFoot}>
                  ...
                  <div style={S.actions}>
                    <button onClick={() => handleContinue(en)} style={{ ...S.continueBtn, opacity: (isDeleted || isUnpublished) ? 0.5 : 1 }}>
                      {isDeleted ? 'Course Removed' : isCompleted ? 'Review Course' : 'Continue Learning'}
                    </button>
                    {isCompleted && (
                      cert
                        ? <button onClick={() => navigate('/candidate/my-certificates')} style={S.certBtn}>View Certificate</button>
                        : <button onClick={() => handleClaim(courseId)} disabled={claiming === courseId || !courseId} style={S.claimBtn}>
...
const S = {
  page: { maxWidth: 1320, margin: '0 auto', padding: '2.5rem 2rem', minHeight: '80vh' },
  header: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem', paddingBottom: '1.5rem', borderBottom: '1px solid var(--border)' },
  title: { fontSize: 30, fontWeight: 900, color: 'var(--text-primary)', letterSpacing: '-0.03em' },
  ...
  emptyBtn: { marginTop: '0.5rem', padding: '0.75rem 2rem', background: 'linear-gradient(135deg, #6366f1, #22d3ee)', color: 'white', border: 'none', borderRadius: 10, fontSize: 14, fontWeight: 700, cursor: 'pointer' },
  ...
  cardCover: { width: '100%', height: 130, objectFit: 'cover', display: 'block', flexShrink: 0 },
  ...
  courseTitle: { fontSize: 16, fontWeight: 700, color: 'var(--text-primary)', flex: 1, lineHeight: 1.4 },
  badges: { display: 'flex', flexDirection: 'column', gap: 4, alignItems: 'flex-end', flexShrink: 0 },
  removedBadge: { fontSize: 10, fontWeight: 700, color: '#f87171', background: 'rgba(239,68,68,0.1)', border: '1px solid rgba(239,68,68,0.2)', borderRadius: 99, padding: '2px 8px' },
  statusBadge: { fontSize: 10, fontWeight: 600, borderRadius: 99, padding: '2px 8px', textTransform: 'capitalize' },
  ...
  cardFoot: { padding: '0.875rem 1.25rem 1.25rem', borderTop: '1px solid var(--border)', display: 'flex', flexDirection: 'column', gap: '0.875rem' },
  ...
  continueBtn: { flex: 1, padding: '0.6rem', background: 'rgba(255,255,255,0.04)', color: 'var(--text-secondary)', border: '1px solid var(--border)', borderRadius: 8, fontSize: 12, fontWeight: 600, cursor: 'pointer' },
  claimBtn: { flex: 1, padding: '0.6rem', background: 'linear-gradient(135deg, #10b981, #059669)', color: 'white', border: 'none', borderRadius: 8, fontSize: 12, fontWeight: 700, cursor: 'pointer' },
  certBtn: { flex: 1, padding: '0.6rem', background: 'linear-gradient(135deg, #6366f1, #22d3ee)', color: 'white', border: 'none', borderRadius: 8, fontSize: 12, fontWeight: 700, cursor: 'pointer' },
  ...
  modal: { background: 'var(--bg-elevated)', border: '1px solid var(--border)', borderRadius: 20, padding: '2.5rem', maxWidth: 400, width: '90%', textAlign: 'center', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '0.875rem' },
  modalIconWrap: { width: 56, height: 56, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center' },
  modalTitle: { fontSize: 19, fontWeight: 700, color: 'var(--text-primary)' },
  modalMsg: { fontSize: 14, color: 'var(--text-secondary)', lineHeight: 1.65, marginBottom: '0.5rem' },
  modalBtn: { padding: '0.75rem 2rem', background: 'linear-gradient(135deg, #6366f1, #22d3ee)', color: 'white', border: 'none', borderRadius: 10, fontWeight: 700, fontSize: 14, cursor: 'pointer' },
};
