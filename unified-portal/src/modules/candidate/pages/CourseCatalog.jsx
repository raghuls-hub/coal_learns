          ...
          {options.map((opt) => (
            <div
              ...
              style={{
                ...S.dropdownItem,
                background:
                  value === opt.value ? "rgba(99,102,241,0.12)" : "transparent",
                color:
                  value === opt.value ? "#818cf8" : "var(--text-secondary)",
              }}
            >
              {opt.label}
            </div>
...
export default function CourseCatalog() {
  ...
  useEffect(() => {
    apiClient
      ...
      .then((res) =>
        setCourses(
          (res.data.data?.courses || res.data.data || []).filter(
            (c) => c.settings?.isPublished,
          ),
        ),
      )
      .catch(() => {})
  ...
  const hasFilters =
    search || level !== "all" || priceIdx !== 0 || category !== "all";
    ...
    <div style={S.page}>
      ...
      <div style={S.banner}>
        ...
        <div style={S.bannerInner}>
          ...
          <p style={S.bannerSub}>
            Handcrafted learning paths designed to build real-world skills.
          </p>
          {/* Main search */}
      ...
      <div style={S.body}>
        ...
        {loading ? (
          ...
          <div style={S.empty}>
            ...
            {hasFilters && (
              <button onClick={clearAll} style={S.emptyBtn}>
                Clear all filters
              </button>
            )}
          </div>
          ...
          <div style={S.grid}>
            {filtered.map((course) => {
              const lc = LEVEL_META[course.level] || {
              ...
              return (
                <div
                  key={course._id}
                  style={S.card}
                  onClick={() => navigate(`/candidate/course/${course._id}`)}
                >
                  {/* Cover */}
