...
const formatCourse = (row) => ({
  ...
  tutors: row.tutors || [],
  modules: row.modules || [],
  pricing: { amount: parseFloat(row.price_amount), currency: row.price_currency },
  settings: {
...
const attachTutors = async (courseIds) => {
  ...
  for (const r of rows) {
    ...
    map[r.course_id].push({
      _id: r.id, id: r.id, email: r.email,
...
exports.createCourse = async (courseData, courseHandlerId) => {
  const { title, description, thumbnail, coverImage, category, level, pricing, settings } = courseData;
  ...
  try {
    ...
    const { rows } = await client.query(
      `INSERT INTO courses
       ...
       RETURNING *`,
      [
        title, description, thumbnail || null, coverImage || null,
        category, level || 'beginner', courseHandlerId,
        pricing?.amount ?? 0, pricing?.currency || 'USD',
...
exports.getCourses = async (filters = {}, page = 1, limit = 10) => {
  ...
  if (filters.level)    { conditions.push(`c.level = $${i++}`);    params.push(filters.level); }
  if (filters.courseHandler) { conditions.push(`c.course_handler_id = $${i++}`); params.push(filters.courseHandler); }
  if (filters.isPublished !== undefined) { conditions.push(`c.is_published = $${i++}`); params.push(filters.isPublished); }
  if (filters.search) {
    conditions.push(`to_tsvector('english', c.title || ' ' || c.description) @@ plainto_tsquery('english', $${i++})`);
...
exports.updateCourse = async (courseId, updateData, userId, userRole) => {
  ...
  const fieldMap = {
    ...
    coverImage: 'cover_image', category: 'category', level: 'level',
  };
  const fields = [];
  const params = [];
  let i = 1;
  ...
  for (const [key, col] of Object.entries(fieldMap)) {
    if (updateData[key] !== undefined) { fields.push(`${col} = $${i++}`); params.push(updateData[key]); }
  }
  if (updateData.pricing?.amount !== undefined) { fields.push(`price_amount = $${i++}`); params.push(updateData.pricing.amount); }
  if (updateData.pricing?.currency)             { fields.push(`price_currency = $${i++}`); params.push(updateData.pricing.currency); }
  if (updateData.settings?.isPublished !== undefined) {
    fields.push(`is_published = $${i++}`); params.push(updateData.settings.isPublished);
    fields.push(`is_archived = $${i++}`); params.push(!updateData.settings.isPublished);
  }
  if (updateData.settings?.passingPercentage !== undefined) {
    fields.push(`passing_percentage = $${i++}`); params.push(updateData.settings.passingPercentage);
  }
  if (updateData.settings?.enrollmentLimit !== undefined) {
    fields.push(`enrollment_limit = $${i++}`); params.push(updateData.settings.enrollmentLimit);
  }
...
exports.deleteCourse = async (courseId, userId, userRole) => {
  ...
  const { rowCount } = await pool.query(
    `UPDATE enrollments SET
     ...
     WHERE course_id = $7`,
    [course.title, course.description, course.thumbnail, course.category, course.level, instructorName, courseId]
  );
...
exports.addTutor = async (courseId, tutorId, userId, userRole) => {
  ...
  await pool.query('INSERT INTO course_tutors (course_id, user_id) VALUES ($1,$2)', [courseId, tutorId]);
...
exports.removeTutor = async (courseId, tutorId, userId, userRole) => {
  ...
  await pool.query('DELETE FROM course_tutors WHERE course_id=$1 AND user_id=$2', [courseId, tutorId]);
  return exports.getCourseById(courseId);
};
...
exports.getModulesByCourseId = async (courseId) => {
  ...
  return rows.map((r) => ({ ...formatModule(r), content: contentMap[r.id] || [] }));
};

exports.getModuleById = async (moduleId) => {
  const { rows } = await pool.query('SELECT * FROM modules WHERE id = $1', [moduleId]);
...
exports.createModule = async (courseId, moduleData) => {
  const { rows: course } = await pool.query('SELECT id FROM courses WHERE id = $1', [courseId]);
  ...
  const { rows } = await pool.query(
    ...
    [
      courseId, title, description || null, order, duration || null,
...
exports.updateModule = async (moduleId, updateData) => {
  const { rows: existing } = await pool.query('SELECT id FROM modules WHERE id = $1', [moduleId]);
  if (!existing.length) throw new Error('Module not found');

  const fieldMap = { title: 'title', description: 'description', order: '"order"', duration: 'duration' };
  const fields = [];
  const params = [];
  let i = 1;
...
exports.deleteModule = async (courseId, moduleId) => {
  const { rows } = await pool.query('SELECT id FROM modules WHERE id = $1', [moduleId]);
  if (!rows.length) throw new Error('Module not found');
...
exports.addContentToModule = async (moduleId, contentData) => {
  const { rows: mod } = await pool.query('SELECT id FROM modules WHERE id = $1', [moduleId]);
  ...
  const { rows } = await pool.query(
    ...
    [
      moduleId, type, title, description || null, order,
      data?.url || null, data?.duration || null, data?.size || null,
      data?.htmlContent || null, data?.externalUrl || null,
    ]
  );
...
exports.updateContent = async (contentId, updateData) => {
  const { rows: existing } = await pool.query('SELECT * FROM content WHERE id = $1', [contentId]);
  ...
  const topMap = { title: 'title', description: 'description', order: '"order"', type: 'type' };
  ...
  if (updateData.data) {
    ...
    if (updateData.data.url && cur.url && updateData.data.url !== cur.url) {
      ...
      fields.push(`version = $${i++}`); params.push((cur.version || 1) + 1);
    }
    const dataMap = { url: 'url', duration: 'duration', size: 'file_size', htmlContent: 'html_content', externalUrl: 'external_url' };
    for (const [key, col] of Object.entries(dataMap)) {
      if (updateData.data[key] !== undefined) { fields.push(`${col} = $${i++}`); params.push(updateData.data[key]); }
    }
  }
...
exports.deleteContent = async (contentId) => {
  const { rows } = await pool.query('DELETE FROM content WHERE id = $1 RETURNING id', [contentId]);
